...
let numberMutationSpan;
let modeTitleSpan

// All time high score
let highScore = 0;
let numberMutation = 0;
...
function setup() {
  let canvas = createCanvas(1120, 630);
  ...
  numberMutationSpan = select('#nm');
  modeTitleSpan = select('#mode');
  runBestButton = select('#best');  
  runBestButton.mousePressed(toggleState1);
  runStopButton = select('#stop');  
  runStopButton.mousePressed(toggleState2);
...
function toggleState1() {
  ...
  if (runBest) {
    ...
    modeTitleSpan.html('Testing!');

    // Go train some more
  } else {
    nextGeneration();
    runBestButton.html('Run Best');
    modeTitleSpan.html('Training!');
  } 
...
function toggleState2() { 
  ...
  if (runStop) {
    resetGame();
    noLoop();
    runStopButton.html('Restart');
  } else {
    loop();
    runStopButton.html('Stop Game');
  }
}
...
function draw() {
  ...
  numberMutationSpan.html(numberMutation);
